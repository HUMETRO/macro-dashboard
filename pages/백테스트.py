...
st.set_page_config(page_title="V8 Ultimate 하이브리드", page_icon="🚦", layout="wide")
...
st.title("🚦 V8 하이브리드 Ultimate: 정밀 정정 버전")
st.caption("Adj Close 기반 데이터 정제 및 Trailing Stop 로직 통일을 통해 SOXL 오류를 완벽 해결했습니다.")
...
EVENTS = [
    ...
    {"date": "2022-01-05", "name": "인플레이션 하락장", "type": "danger", "desc": "금리 인상기 하락장 회피"}
]

# ── [개선] 데이터 로딩 (Adj Close 사용 및 이상치 정제) ──
@st.cache_data(ttl=3600, show_spinner=False)
def load_ultimate_data(ticker, start_year):
    fetch_start = f"{start_year - 1}-01-01"
    df = yf.download(ticker, start=fetch_start, interval='1d', progress=False)
    if isinstance(df.columns, pd.MultiIndex): df.columns = df.columns.get_level_values(0)
    
    # 💡 다른 쌤의 조언: 분할 문제 해결을 위해 Adj Close 최우선 사용
...
# ── [개선] 성과 계산 (다른 쌤의 정제 로직 반영) ──
def calc_ultimate_returns(df, ticker, start_year):
    if df.empty: return None
    df = df[df.index >= f"{start_year}-01-01"].copy()
    
    # 💡 데이터 클리핑: 비정상 수익률(-100% 이하 등) 방지
    df['daily_ret'] = df['Close'].pct_change().replace([np.inf, -np.inf], np.nan)
    df['daily_ret'] = df['daily_ret'].clip(lower=-0.99, upper=10.0).fillna(0)
    ...
    # 💡 다른 쌤의 조언: DD 계산과 실제 수익률 경로 통일
    final_exp = []
    cur_cum, max_cum = 1.0, 1.0
    
    for i in range(len(df)):
        exp, d_ret = df['base_exp'].iloc[i], df['daily_ret'].iloc[i]
        # 일단 현재 비중으로 가상 수익 계산하여 DD 확인
        temp_cum = cur_cum * (1 + d_ret * exp)
        dd = (temp_cum / max_cum) - 1
        
        # 세이프가드 발동 여부 결정
        actual_exp = exp * 0.3 if dd < -0.08 else exp
        
        # 💡 실제 수익률은 확정된 actual_exp로 업데이트
        cur_cum *= (1 + d_ret * actual_exp)
...
raw_data = load_ultimate_data(ticker, start_year)
sig_df = calculate_v8_signals(raw_data, ticker)
perf_df = calc_ultimate_returns(sig_df, ticker, start_year)
...
if perf_df is not None:
    ...
    st.plotly_chart(go.Figure([
        go.Scatter(x=perf_df.index, y=(perf_df['cum_strat']-1)*100, name='전략'),
        go.Scatter(x=perf_df.index, y=(perf_df['cum_bah']-1)*100, name='존버', line=dict(dash='dot'))
    ]).update_layout(title=f"{ticker} 누적 수익률 비교(%)", height=450), use_container_width=True)
