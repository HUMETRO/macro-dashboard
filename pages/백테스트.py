...
st.set_page_config(page_title="V8 순정 복구 리포트", page_icon="🛡️", layout="wide")
...
st.title("🛡️ V8 하이브리드: 소장님 원픽 완성본")
st.caption("모든 오류를 걷어내고 소장님이 가장 신뢰하셨던 순정 로직과 지표 배치를 완벽 복구했습니다.")
...
EVENTS = [{"date": "2000-03-24", "name": "닷컴버블"}, {"date": "2008-09-15", "name": "리먼파산"}, {"date": "2020-02-24", "name": "코로나쇼크"}, {"date": "2022-01-05", "name": "인플레하락"}]
...
for i, ev in enumerate(EVENTS):
    ...
    with ev_cols[i % 2]:
        st.markdown(f'<div class="event-card"><b>📅 {ev["date"]} | {ev["name"]}</b><br>신호: {after_data["신호"].iloc[0]}<br><span class="win-text">전략: {s_p:.1f}%</span> / <span class="loss-text">존버: {b_p:.1f}%</span></div>', unsafe_allow_html=True)
