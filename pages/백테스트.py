...
st.set_page_config(page_title="V8 데이터 정합성 리포트", page_icon="🛡️", layout="wide")
...
st.title("🛡️ V8 하이브리드: 데이터 정합성 리포트")
st.caption("소장님의 지시대로 QQQ 우선 배치 및 TQQQ의 2010년 실전 데이터 고정을 완료했습니다.")
...
# ── 1. 종목 선택 및 연도 설정 (순서 재배치) ──
# 소장님 요청: QQQ부터 확인할 수 있게 배치
ticker = st.selectbox("종목 선택 (QQQ 우선 확인)", ["QQQ", "TQQQ", "SOXX", "QLD", "SPY"])

# 💡 소장님 요청: TQQQ 선택 시 2010년으로 강제 고정 로직
if ticker == "TQQQ":
...
# 실행
raw_data = load_v8_final_data(ticker, start_year)
...
for i, ev in enumerate(EVENTS):
    ...
    with ev_cols[i % 2]:
        st.markdown(f'<div class="event-card"><b>📅 {ev["date"]} | {ev["name"]}</b><br><span class="win-text">전략: {s_p:.1f}%</span> / <span class="loss-text">존버: {b_p:.1f}%</span><br><b>🛡️ 방어 우위: {s_p - b_p:.1f}%p</b></div>', unsafe_allow_html=True)
