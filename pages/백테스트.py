...
st.set_page_config(page_title="V8 최종 안정화 리포트", page_icon="🛡️", layout="wide")
...
st.title("🛡️ V8 하이브리드: 최종 안정화 리포트")
st.caption("수익률 폭주 오류를 해결하고 소장님의 지시대로 지표를 재배치한 완성형 버전입니다.")
...
for i, ev in enumerate(EVENTS):
    ...
    with ev_cols[i % 2]:
        ...
        <div class="event-card">
            ...
            <div class="metric-text">신호 후 60일 존버: <span class="loss-text">{b_p:.1f}%</span></div>
            <div class="metric-text">신호 후 60일 전략: <span class="win-text">{s_p:.1f}%</span></div>
            <div class="metric-text"><b>🛡️ 방어 성공: {s_p - b_p:.1f}%p 보호</b></div>
        </div>
        """, unsafe_allow_html=True)
