...
import pandas as pd
import numpy as np
import yfinance as yf # VIX, 금리차 조회용
...
if selected:
    hist, ticker = all_data['sector_etfs'][selected]['history'].copy(), all_data['sector_etfs'][selected]['ticker']
    if isinstance(hist.columns, pd.MultiIndex): hist.columns = hist.columns.get_level_values(0)
    def to_1d(col):
        s = hist[col]
        return s.iloc[:, 0].values.flatten() if isinstance(s, pd.DataFrame) else s.values.flatten()
    ...
    fig.update_layout(title=f"{selected} ({ticker}) 분석 차트", template="plotly_white", height=450, xaxis_range=[date_list[-view_days], date_list[-1]], hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(l=10, r=10, t=50, b=10))
