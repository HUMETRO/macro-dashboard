import streamlit as st
import plotly.graph_objects as go
import sys
import os
import pandas as pd
import numpy as np

# [1] 경로 설정
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(current_dir)
if parent_dir not in sys.path:
    sys.path.append(parent_dir)

# [2] 부품 로드
try:
    from data_fetcher import get_all_market_data
    from calculations import calculate_sector_scores, calculate_individual_metrics, calculate_core_sector_scores
except ImportError as e:
    st.error(f"🚨 부품 로딩 실패! (에러: {e})")
    st.stop()

st.set_page_config(page_title="매크로 위험알리미", page_icon="📊", layout="wide")

# 🎨 스타일 CSS (공간 효율 및 가독성 강화)
st.markdown("""
    <style>
    .metric-card {
        background-color: #ffffff;
        border-radius: 8px;
        padding: 10px;
        border: 1px solid #e5e7eb;
        margin-bottom: 5px;
        min-height: 90px;
    }
    .buy-signal { border-left: 5px solid #10b981; background-color: #f0fdf4; }
    .sell-signal { border-left: 5px solid #ef4444; background-color: #fef2f2; }
    .wait-signal { border-left: 5px solid #f59e0b; background-color: #fffbeb; }
    .ticker-header { font-size: 0.85rem; font-weight: 700; color: #111827 !important; margin-bottom: 2px; }
    .score-box { font-size: 0.75rem; color: #374151 !important; line-height: 1.3; }
    </style>
    """, unsafe_allow_html=True)

st.title("📊 매크로경제 위험알리미")
st.markdown("---")

# [3] 데이터 로딩
@st.cache_data(ttl=300)
def load_all_data():
    return get_all_market_data()

with st.spinner("⏳ 실시간 분석 중..."):
    all_data = load_all_data()
    df_sectors = calculate_sector_scores(all_data['sector_etfs'])
    df_individual = calculate_individual_metrics(all_data['individual_stocks'])
    df_core = calculate_core_sector_scores(all_data['core_sectors'])

# [4] 메인 시장 상태 지표 (도망챠! 유지)
if not df_sectors.empty:
    c1, c2, c3 = st.columns(3)
    avg_l, avg_s = df_sectors['L-score'].mean(), df_sectors['S-score'].mean()
    with c1: st.metric("평균 L-score", f"{avg_l:.2f}", delta="장기 체력", delta_color="off")
    with c2: st.metric("평균 S-score", f"{avg_s:.2f}", delta="단기 기세", delta_color="off")
    with c3:
        if avg_l > 0 and avg_s > 0: st.success("✅ 매수 신호 (상승장)")
        elif avg_l < 0 and avg_s < 0: st.error("🚨 도망챠! (하락장)")
        else: st.warning("⚠️ 관망 (방향 탐색)")
    st.caption("💡 판별 기준: 전체 평균 장기/단기 스코어가 모두 (+)면 '매수', (-)면 '도망챠!', 그 외는 '관망'입니다. 객관적 숫자를 믿으십시오.")

# [5] 조기경보 시스템
top_5 = df_sectors.head(5)['섹터'].tolist()
safe_assets = ['CASH', '장기국채', '물가연동채', '유틸리티', '필수소비재']
safe_count = sum(1 for s in top_5 if s in safe_assets)
if safe_count >= 2:
    st.error(f"🚨 **안전자산 쏠림 경보!** 상위 5개 중 {safe_count}개가 피난처 자산입니다. 주식 비중 확대를 멈추십시오!")

st.markdown("---")

# [6] 메인 탭 구성
tab1, tab2, tab3 = st.tabs(["📈 섹터 ETF", "💹 개별 종목", "🎯 핵심 11"])

with tab1:
    st.subheader("📈 섹터 ETF 분석")
    # ⭐ 제자님 요청: 테이블 뷰를 먼저 보이게 배치
    sub_t, sub_c = st.tabs(["📑 테이블 뷰 (정밀 분석)", "🎴 카드 뷰 (기세 확인)"])
    
    with sub_t:
        def hb(row):
            s = row['섹터']
            if s in ['S&P', 'NASDAQ']: return ['background-color: #d9d9d9; font-weight: bold'] * len(row)
            elif s in ['CASH', '물가연동채', '장기국채']: return ['background-color: #e2efda; color: #385723; font-weight: bold'] * len(row)
            return [''] * len(row)
        st.dataframe(df_sectors.style.apply(hb, axis=1)
                     .background_gradient(cmap='RdYlGn', subset=['L-score', 'S-score', 'S-L', '20일(%)'])
                     .format({'L-score': '{:.2f}', 'S-score': '{:.2f}', 'S-L': '{:.2f}', '20일(%)': '{:.2f}%'}),
                     use_container_width=True, height=450)

    with sub_c:
        # ⭐ 제자님 요청: 한 줄에 4개씩 배치하여 스크롤 단축
        cols = st.columns(4) 
        for idx, row in df_sectors.iterrows():
            with cols[idx % 4]:
                sig = "buy-signal" if row['S-score'] > 0 and row['L-score'] > 0 else \
                      "sell-signal" if row['S-score'] < 0 and row['L-score'] < 0 else "wait-signal"
                st.markdown(f"""
                    <div class="metric-card {sig}">
                        <div class="ticker-header">{row['섹터']} ({row['티커']})</div>
                        <div class="score-box">
                            <b>S-L: {row['S-L']}</b> | <b>{row['20일(%)']}%</b><br>
                            L:{row['L-score']} / S:{row['S-score']}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)

    # 기존 설명 문구 유지
    st.markdown("##### 💡 퀀트 지표 핵심 요약")
    st.caption("**📊 L-score**: 장기 추세 / **🚀 S-score**: 단기 모멘텀 / **1️⃣ S-L**: 추세 가속도")
    st.caption("**2️⃣ 미너비니 필터**: 단기 추세(-) 섹터는 가짜 신호로 간주하여 강제 강등")

with tab2:
    st.subheader("💹 개별 종목 현황")
    st.dataframe(df_individual.style.background_gradient(cmap='RdYlGn', subset=['연초대비', 'high대비', '200대비', '52저대비'])
                 .format({'현재가': '{:.2f}', '연초대비': '{:.1f}%', 'high대비': '{:.1f}%', '200대비': '{:.1f}%', '전일대비': '{:.1f}%', '52저대비': '{:.1f}%'}),
                 use_container_width=True, height=450)

with tab3:
    st.subheader("🎯 핵심 11 섹터")
    st.dataframe(df_core.style.background_gradient(cmap='RdYlGn', subset=['S-SCORE', '20일(%)'])
                 .format({'S-SCORE': '{:.2f}', '20일(%)': '{:.2f}%'}),
                 use_container_width=True, height=450)

# [7] 차트
st.markdown("---")
selected = st.selectbox("📉 상세 차트 선택", list(all_data['sector_etfs'].keys()))
if selected:
    hist = all_data['sector_etfs'][selected]['history'].copy()
    if isinstance(hist.columns, pd.MultiIndex): hist.columns = hist.columns.get_level_values(0)
    date_list = hist.index.tolist()
    def gv(s): return s.values.flatten() if isinstance(s, pd.DataFrame) else s.values

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=date_list, y=gv(hist['Close']), name='종가', line=dict(color='blue', width=2)))
    if 'MA20' in hist.columns: fig.add_trace(go.Scatter(x=date_list, y=gv(hist['MA20']), name='MA20', line=dict(dash='dash', color='orange')))
    if 'MA200' in hist.columns: fig.add_trace(go.Scatter(x=date_list, y=gv(hist['MA200']), name='MA200', line=dict(dash='dot', color='green', width=2)))
    
    fig.update_layout(title=f"{selected} 분석 차트", template="plotly_white", height=450, hovermode="x unified")
    st.plotly_chart(fig, use_container_width=True)
