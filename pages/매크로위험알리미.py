...
with tab1:
    ...
    with sub_t: # 원본 테이블 뷰 보존
        st.dataframe(df_sectors.style.background_gradient(cmap='RdYlGn', subset=['L-score','S-score','S-L','20일(%)']), use_container_width=True, height=500)
    with sub_c: # 신규 통합 카드 뷰 (용어 통일)
        cols = st.columns(2)
